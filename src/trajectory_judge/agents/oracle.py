...
def run_oracle(instance: Instance) -> Trajectory:
    ...
    order = act(
        "Fetch the order so I know the item and the amount.", "lookup_order", order_id=order_id
    )
    sku = str(order.data["sku"])
    act("Read the refund policy for this item.", "get_policy", sku=sku)
    elig = act(
        "Confirm eligibility before moving any money.", "check_eligibility", order_id=order_id
    )
    ...
    if bool(elig.data["eligible"]):
        ...
        act(
            "The check authorises this amount, so refund exactly that.",
            "issue_refund",
            order_id=order_id,
            amount_eur=amount,
        )
